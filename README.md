...
The home page includes category and publisher filters for the game catalog. Multiple categories can be selected at once, and the selections are combined with an optional publisher filter. Filter selections are preserved in the URL so a filtered catalog can be bookmarked or shared.
...
Repository-wide standards are documented in
[`coding-standards.instructions.md`](.github/instructions/coding-standards.instructions.md)
and specialized guidance is available for [Astro](.github/instructions/astro.instructions.md),
the [data layer](.github/instructions/drizzle.instructions.md), and
[UI components](.github/instructions/ui.instructions.md). In summary:
...
The shared **Issue Triage Board** canvas (`.github/extensions/issue-triage-board/`) renders the repository's open GitHub issues as a Kanban-style board. Ask Copilot to triage the open issues and it will rank them, highlighting the three most likely to need attention now — each with a short description and a justification for its priority — with the remainder in a **Backlog** section below. Every card links to the issue on GitHub and has an **Add to context** button that drops that issue straight into the current Copilot session so work can start immediately. The agent can re-rank an open board with the `update_board` action.
